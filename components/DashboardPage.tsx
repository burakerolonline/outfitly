...
import { generateMockAnalysis, generateMockOutfits } from "@/lib/constants";
...
export default function DashboardPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const handleFile = useCallback((e: React.DragEvent | React.ChangeEvent<HTMLInputElement>) => {
    e.preventDefault();
  ...
  const handleProductFile = useCallback((e: React.DragEvent | React.ChangeEvent<HTMLInputElement>) => {
    e.preventDefault();
    const file = "dataTransfer" in e ? e.dataTransfer?.files?.[0] : e.target?.files?.[0];
    if (file && file.type.startsWith("image/")) {
      const reader = new FileReader();
  ...
  const handleGenerate = async () => {
    ...
    // Simulated AI pipeline - replace with real API calls
    ...
    setGenStep(3);
    const outfits = generateMockOutfits(selectedStyle);
    setOutfits(outfits);
    ...
    >
        ...
        {!uploadedImage ? (
          ...
          <div className="relative inline-block rounded-[20px] overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            ...
            <button
              ...
              className="absolute top-3 right-3 w-9 h-9 rounded-[10px] flex items-center justify-center border-none cursor-pointer text-white"
          ...
          {STYLES.map((s) => {
            ...
            return (
              ...
              >
                {active && (
                  <div
                    className="absolute top-2.5 right-2.5 w-[22px] h-[22px] rounded-md flex items-center justify-center text-white"
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-12">
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <span
            className="text-xs font-normal px-2.5 py-0.5 rounded-md"
          ...
          <div>
            {!productImage ? (
              <div
                ...
                style={{ border: "1px dashed var(--border)", padding: 28, background: "var(--bg-card)" }}
              >
              ...
              <div className="relative rounded-[14px] overflow-hidden" style={{ border: "1px solid var(--border)" }}>
                ...
                <button
                  ...
                  className="absolute top-2 right-2 w-7 h-7 rounded-lg flex items-center justify-center border-none cursor-pointer text-white"
                  style={{ background: "rgba(0,0,0,0.6)" }}
                >
                  <X size={14} />
                </button>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        {!generating ? (
          <button
            className="btn-primary w-full !py-[18px] !text-[17px] !rounded-2xl flex items-center justify-center gap-2.5"
            onClick={handleGenerate}
            disabled={!canGenerate}
          >
            <Sparkles size={18} /> Generate Outfit
            {user && user.credits <= 0 && user.plan === "free" && (
              <Lock size={14} className="ml-2 opacity-70" />
            )}
          </button>
        ) : (
          <div
          ...
          >
            <div className="mb-5 inline-block relative">
              <motion.div
                animate={{ rotate: 360 }}
            ...
            <div className="flex gap-1.5 justify-center">
              {genSteps.map((_, i) => (
                <div
                  ...
                  style={{
                    width: i <= genStep ? 32 : 20,
                    background: i <= genStep ? "var(--accent)" : "var(--border)",
                  }}
                />
              ))}
            </div>
